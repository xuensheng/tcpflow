...
#access_key_id = 'LTAIpcydYJupS4kl'
#access_key_secret = 'AAGBNjIdJeo2dGv5S65UPhppuHRSRr'
#ram
#access_key_id = 'LTAIoDlTQdNFRjvu'
#access_key_secret = '8shpsVmXBT929XEWqJWNyhCuRwRTb7'
#test
...
print '==========Live Channel Info========'
print 'Desc: %s' % (get_result.description)
print 'Status: %s' % (get_result.status)
print 'Type: %s' % (get_result.target.type)
print 'Count: %s' % (get_result.target.frag_count)
print 'Duration: %s' % (get_result.target.frag_duration)
print 'Playlist: %s' % (get_result.target.playlist_name)
...
print '==========Live Channel Playlist========'
print 'live channel playlist: %s.%s/%s/%s' % (bucket_name, endpoint, channel_name, playlist_name)

publish_cmd = 'ffmpeg -re -i %s -c copy -bsf:a aac_adtstoasc -f flv "%s"' % (sys.argv[1], signed_url)
print publish_cmd
exit(0)

status, output = commands.getstatusoutput(publish_cmd)
print 'publish status: %s' % (status)
...
print 'live channel playlist: %s.%s/%s/%s' % (bucket_name, endpoint, channel_name, 'vod.m3u8')
...
while True:
    ...
    print '==========Live Channel Status========'
    print 'status: %s' % (stat_result.status)
    if stat_result.status == 'Live':
        print 'remote addr: %s' % (stat_result.remote_addr)
        print 'connectd time: %s' % (stat_result.connected_time)

        print 'Video width      : %d' % stat_result.video.width
        print 'Video height     : %d' % stat_result.video.height
        print 'Video frame rate : %d' % stat_result.video.frame_rate
        print 'Video codec      : %s' % stat_result.video.codec
        print 'Video bandwidth  : %d' % stat_result.video.bandwidth

        print 'Audio codec      : %s' % stat_result.audio.codec
        print 'Audio sample rate: %d' % stat_result.audio.sample_rate
        print 'Audio bandwidth  : %d' % stat_result.audio.bandwidth
    else:
        break
    time.sleep(1)
